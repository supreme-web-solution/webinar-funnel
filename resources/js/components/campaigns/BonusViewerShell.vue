...
import type { ViewerSlide } from '@/composables/useBonusViewerSlides';
...
onMounted(() => {
    if (props.embedded) document.addEventListener('keydown', onKey);
});

onUnmounted(() => {
    if (props.embedded) document.removeEventListener('keydown', onKey);
});
</script>

<template>
    <div class="course-app" :class="{ 'course-app--embedded': embedded }">
        ...
        >
            <div v-if="current?.kind === 'cover'" class="slide-cover">
                ...
                <span class="preview-badge">
                    ...
                    {{ bonusType === 'mini_course' ? 'Course preview' : 'Ebook preview' }}
                </span>
                ...
                <a v-if="downloadUrl" :href="downloadUrl" class="download-btn" target="_blank" rel="noopener">
                    Download PDF
                </a>
            </div>
        ...
        <footer class="course-footer">
            <button type="button" class="btn-ghost" :disabled="index === 0" @click="go(-1)">
...
.course-app--embedded .course-main {
    min-height: 320px;
    max-height: 480px;
    overflow-y: auto;
}

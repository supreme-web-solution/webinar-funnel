    ...
    <Dialog :open="open">
        <DialogContent class="gap-0 overflow-hidden rounded-xl border border-border/60 p-0 shadow-xl sm:max-w-xl" :show-close-button="false">
            ...
            <div v-if="step === 'type'" class="grid gap-3 sm:grid-cols-2">
                <button
                    type="button"
                    class="rounded-xl border-2 border-border/60 bg-white p-5 text-left shadow-sm transition-all hover:border-teal-400 hover:shadow-md"
                    @click="pickType('sales')"
                >
                    <Icon icon="heroicons:shopping-bag" class="mb-3 size-9 text-teal-600" />
                ...
                >
                    <Icon icon="heroicons:video-camera" class="mb-3 size-9 text-teal-600" />
            ...
            <div v-else class="space-y-3">
                <button
                    type="button"
                    class="flex w-full items-start gap-4 rounded-xl border-2 border-border/60 bg-white p-4 text-left shadow-sm transition-all hover:border-teal-400 hover:shadow-sm"
                    @click="pickSource('links')"
                >
                    <Icon icon="heroicons:link" class="mt-0.5 size-8 shrink-0 text-teal-600" />
                    <div>
                ...
                >
                    <Icon icon="heroicons:magnifying-glass" class="mt-0.5 size-8 shrink-0 text-teal-600" />
                    <div>
                ...
                <Button variant="ghost" size="sm" class="mt-2 text-teal-700 hover:text-teal-800" @click="goBack">← Back</Button>
            </div>
            </div>
        </DialogContent>
    </Dialog>
